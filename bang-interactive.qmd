...
const GLOBE_CONFIG = {
    ...
    SAT_TEXTURE_URL: "https://mid.frmai.life/images/earth-blue-marble-maritime.png",
...
const MIDDLE_POWERS = {
  ...
  "PRK": { label: "🇰🇵 北韓 (North Korea)", name: "North Korea", lat: 40.0, lon: 127.0, color: 0xD73027, closeDistF: 1.40, type: "非選舉威權", middlep: "🔑中關" },
  ...
  "VNM": { label: "🇻🇳 越南 (Vietnam)", name: "Vietnam", lat: 16.0, lon: 108.0, color: 0xD73027, closeDistF: 1.70, type: "非選舉威權", middlep: "🔑中關" },
  ...
  "COL": { label: "🇨🇴 哥倫比亞 (Colombia)", name: "Colombia", lat: 4.0, lon: -73.0, color: 0x8dc1dc, closeDistF: 1.80, type: "選舉式民主", middlep: "🔑中關" },

  // === 🗺️ 中地強權 (REGIONAL MIDDLE POWERS - 13) ===
  "ARG": { label: "🇦🇷 阿根廷 (Argentina)", name: "Argentina", lat: -34.0, lon: -64.0, color: 0x8dc1dc, closeDistF: 1.70, type: "選舉式民主", middlep: "🗺️中地" },
  ...
  "SAU": { label: "🇸🇦 沙烏地阿拉伯 (Saudi Arabia)", name: "Saudi Arabia", lat: 25.0, lon: 45.0, color: 0xD73027, closeDistF: 1.65, type: "非選舉威權", middlep: "🗺️中地" },
  ...
  "GBR": { label: "🇬🇧 英國 (United Kingdom)", name: "United Kingdom", lat: 55.0, lon: -3.0, color: 0x2c7bb6, closeDistF: 1.3, type: "完全民主", middlep: "⏫中上" },
  "FRA": { label: "🇫🇷 法國 (France)", name: "France", lat: 46.22, lon: 2.2, color: 0x2c7bb6, closeDistF: 1.3, type: "完全民主", middlep: "⏫中上" },
  "RUS": { label: "🇷🇺 俄羅斯 (Russia)", name: "Russia", lat: 60.0, lon: 90.0, color: 0xD73027, closeDistF: 3.00, type: "非選舉威權", middlep: "⏫中上" },
  "DEU": { label: "🇩🇪 德國 (Germany)", name: "Germany", lat: 51.0, lon: 9.0, color: 0x2c7bb6, closeDistF: 1.33, type: "完全民主", middlep: "⏫中上" },
...
```{python}
#| label: tbl-global-powers
#| tbl-cap: "全球中等強權表"
#| echo: false
#| column-sort: true
#| scrolling: true    # Optional: adds horizontal scroll if too wide
...
csv_file_path = "resources/middlep.csv" 

# 2. Load the data, ensuring the boolean columns are correctly typed
try:
    df = pd.read_csv(
        csv_file_path,
        # Ensure 'True' and 'False' are correctly interpreted as boolean
        true_values=['True'], 
        false_values=['False'],
        # Specify dtypes to be certain, especially for boolean columns
        dtype={ 'middlep':'str',
            'ISO2': 'str', 'ISO3': 'str', 'loc_en': 'str', 'loc_zht': 'str', 'emojiflag': 'str', 
            'type': 'str', 'UN_l': 'str', 'UN_s': 'str',
            'g_APEC': 'bool', 'g_G20': 'bool', 'g_OECD': 'bool'
        },
        encoding='utf-8'
    )
    
    # 3. Select and reorder columns for a compact, readable table
    #         'loc_en': '英文名',  
    d_compact_cols = { 'middlep':'權力區','emojiflag': '旗',
        'loc_zht': '中文名', 'type': '🗳政治體制', 
        'ISO2': 'ISO2', 'ISO3': 'ISO3', 
        'UN_l': '◉地域涵括表', 'UN_s': '⦿地域涵括表',
        'g_APEC': '🌐APEC', 'g_G20': '🌐G20', 'g_OECD': '🌐OECD'
    }
    df_compact = df[d_compact_cols.keys()].rename(columns=d_compact_cols)

    # 4. Render the DataFrame as an interactive HTML table.
    # The 'to_html()' method is used inside the code interpreter to show the result,
    # but for true Quarto interactivity, the table is rendered directly.
    # In a Quarto HTML output, just printing the DataFrame will often use 
    # the 'htmltools' package internally, which provides sortability.
    
    # print("## Country Data (Sortable Table)")
    
    # Using the standard Pandas Styler with an appropriate CSS class
    # to hint to Quarto/Jupyter how to render the table interactively.
    styled_table = (
        df_compact.style
        .set_table_attributes('class="dataframe table table-striped table-hover"') 
        .set_caption("全球中等強權表")
        .to_html()
    )
    #df_compact
